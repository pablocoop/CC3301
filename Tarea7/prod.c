...
BigNum *parArrayProd(int a[], int i, int j, int p) {
  ...
  if (i == j) {
    // Utilizando la funcion smallNum (que está en test-prod.c), convertimos el entero a[i] a un BigNum 
    return smallNum(a[i]);
  }
  ...
  if (p == 1) {
    ...
    if (pid == 0) { // Proceso hijo
      ...
      close(fds[0]);
      BigNum *b = parArrayProd(a, i, h, p / 2);
      write(fds[1], &b->n, sizeof(int));
      write(fds[1], b->bits, b->n * sizeof(BigInt_t));
      freeBigNum(b);
      exit(0);
      ...
      close(fds[1]);
      BigNum *c = parArrayProd(a, h + 1, j, p - p / 2);
